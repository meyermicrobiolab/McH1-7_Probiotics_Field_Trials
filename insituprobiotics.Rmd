...
BS1 site field trials Jan 2020 - abandoned due to covid, but got before and after first treatment
```{r, echo=FALSE}
#### 1st sequencing run
...
BS2 site field trials 2020-2021
...
BS3 and MK-48 sites field trials 2021-2022
```{r, echo=FALSE}
#### 1st sequencing run
...
##### combine just BS3 runs
st8 <- readRDS("~/Documents/Github/McH1-7_Probiotics_Field_Trials/cutadapt_NS2661_BS3/seqtab.rds")
st9 <- readRDS("~/Documents/Github/McH1-7_Probiotics_Field_Trials/cutadapt_NS2696_BS3/seqtab.rds")
st.bs3 <- mergeSequenceTables(st8, st9, repeats="sum") # You may get the message "Duplicated sample names detected in the sequence table row names." to let you know that there are duplicate names across samples - it is not an error, just a message. If you have run a sample on more than one sequencing run, the ASV counts will be added together.
#Remove chimeric sequences:
seqtab.nochim.bs3 <- removeBimeraDenovo(st.bs3, method="consensus", multithread=TRUE, verbose=TRUE)
dim(seqtab.nochim.bs3)
sum(seqtab.nochim.bs3)/sum(st.bs3)
saveRDS(seqtab.nochim.bs3, file="~/Documents/Github/McH1-7_Probiotics_Field_Trials/probiotics_bs3.rds")
...
```{r, echo=FALSE}
taxa <- assignTaxonomy(seqtab.nochim.bs3, "~/Documents/Github/silva_nr99_v138.1_train_set.fa.gz", multithread=TRUE)
...
write.table(taxon,"silva_taxa_table_bs3.txt",sep="\t",col.names=NA)
write.table(seqtab.nochim.bs3, "silva_otu_table_bs3.txt",sep="\t",col.names=NA)
# Create phyloseq object from otu and taxonomy tables from dada2, along with the sample metadata.
otu <- read.table("silva_otu_table_bs3.txt",sep="\t",header=TRUE, row.names=1)
taxon <- read.table("silva_taxa_table_bs3.txt",sep="\t",header=TRUE,row.names=1)
...
write.table(otu,"silva_nochloronomito_otu_table_bs3.txt",sep="\t",col.names=NA)
write.table(taxon,"silva_nochloronomito_taxa_table_bs3.txt",sep="\t",col.names=NA)
...
write.table(otu_ra,"silva_nochloronomito_otu_table_bs3_RA.txt",sep="\t",col.names=NA)
```
...
ps #11013 taxa and 229 samples
get_taxa_unique(ps, "Order") #335
get_taxa_unique(ps, "Class") #140
ps5<-filter_taxa(ps, function(x) mean(x) >5, TRUE)
ntaxa(ps5) #227
ps10<-filter_taxa(ps, function(x) mean(x) >10, TRUE)
ntaxa(ps10) #127
get_taxa_unique(ps, "Genus") #1248
get_taxa_unique(ps5, "Genus") #130
get_taxa_unique(ps10, "Genus") #80

# Now export filtered otu and taxa tables from phyloseq for future reference after removing sample blanks
ps5 #227 taxa and 229 samples
ps5 = subset_samples(ps5, tag != "blank")
ps5 #227 taxa and 221 samples
otu_ps5 = as(otu_table(ps5), "matrix")
taxon_ps5 = as(tax_table(ps5), "matrix")
metadata = as(sample_data(ps5), "matrix")
write.table(otu_ps5,"silva_nochloronomito_otu_table_bs3_ps5.txt",sep="\t",col.names=NA)
write.table(taxon_ps5,"silva_nochloronomito_taxa_tabl_bs3_ps5.txt",sep="\t",col.names=NA)
write.table(metadata,"metadata_bs3_noblanks.txt",sep="\t",col.names=NA)
ps5_ra<-transform_sample_counts(ps5, function(OTU) OTU/sum(OTU))
otu_ps5_ra = as(otu_table(ps5_ra), "matrix")
write.table(otu_ps5_ra,"silva_nochloronomito_otu_table_bs3_ps5_RA.txt",sep="\t",col.names=NA)
...
otu <- read.table("silva_nochloronomito_otu_table_bs3_ps5.txt",sep="\t",header=TRUE, row.names=1)
taxon <- read.table("silva_nochloronomito_taxa_tabl_bs3_ps5.txt",sep="\t",header=TRUE,row.names=1)
samples<-read.table("metadata_bs3_noblanks.txt",sep="\t",header=T,row.names=1)
...
pdf("PCA-bs3.pdf",bg ="white",width=8.5)
...
p + labs(x=xlab, y=ylab, fill="Treatment") + coord_fixed()
dev.off()
...
ps5 <- phyloseq(otu_table(otu, taxa_are_rows=FALSE), 
               sample_data(samples), 
               tax_table(taxon))
ps5 #347 taxa and 388 samples
bs3<-subset_samples(ps5, site=="BS3")
bs3  #347 taxa and 219 samples
