...
# reading in the otu table with almost 600 samples took 2 hours!!!
...
All samples from all field trials, using ASV table that has been filtered to remove low-abundance ASVs, no blanks
...
### repeat for only Broward county field trials
...
#calculate multivariate dispersions based on condition
mod2 <-betadisper(dist.clr, treated)
#one way anova
anova(mod2)
#boxplots
plot(mod2)
pdf("distancetocentroid_treated_bs.pdf",width=8.5)
boxplot(mod2)
dev.off()
...
#boxplots
plot(mod3)
pdf("distancetocentroid_condition_bs.pdf",width=8.5)
boxplot(mod3)
dev.off()
```
...
perm<-adonis(dist.clr~treat*month,as(sample_data(ps),"data.frame"))
print(perm)
perm<-adonis(dist.clr~treated*health,as(sample_data(ps),"data.frame"))
print(perm)

# anosim between groups using Aitchison distance
dist.clr <- dist(E.clr)
ano.treated <- anosim(dist.clr, treated, permutations=999)
pdf("anosim_treated_bs3.pdf",width=8.5)
plot(ano.treated)
dev.off()
ano.treat <- anosim(dist.clr, treat, permutations=999)
pdf("anosim_treatment_bs3.pdf",width=8.5)
plot(ano.treat)
dev.off()
ano.health <- anosim(dist.clr, health, permutations=999)
pdf("anosim_condition_bs3.pdf",width=8.5)
plot(ano.health)
dev.off()

## Beta Diversity Dispersions
#calculate multivariate dispersions based on condition
mod <-betadisper(dist.clr, month)
#one way anova
anova(mod)
#boxplots
plot(mod)
pdf("distancetocentroid_month_bs3.pdf",width=8.5)
boxplot(mod)
dev.off()
...
#boxplots
plot(mod2)
pdf("distancetocentroid_treated_bs3.pdf",width=8.5)
boxplot(mod2)
dev.off()
...
#boxplots
plot(mod3)
pdf("distancetocentroid_condition_bs3.pdf",width=8.5)
boxplot(mod3)
dev.off()
...
Perform ANCOM analysis - only BS3 site

```{r, echo=FALSE}

#Read in count data, not relative abundance, no low-abundance ASVs removed - I'm not going to do that right now because it will take 2 hours to load
otu <- read.table("silva_nochloronomito_otu_table_bs3.txt",sep="\t",header=TRUE, row.names=1)
taxon <- read.table("silva_nochloronomito_taxa_table_bs3.txt",sep="\t",header=TRUE,row.names=1)
samples<-read.table("metadata_BS3.txt",sep="\t",header=T,row.names=1)
OTU = otu_table(otu, taxa_are_rows=FALSE)
...
metadat <- select(metadat, c("Sample.ID","treatment","condition","month","treated")) # select only columns of interest
map_test <- metadat
...
#### ANCOM test - Treatment, Adjusted by Condition, ANOVA
comparison_test_treat=ANCOM.main(OTUdat=otu_test, #calling the OTU table
                                 Vardat=map_test, #calling the metadata
                                 adjusted=TRUE, #true if covariates are to be included for adjustment
                                 repeated=FALSE, #repeated measure
                                 main.var="treatment", #main variable or fator
                                 adj.formula= "condition", #other factors to include
                                 repeat.var=FALSE, #repeated measure
                                 long = FALSE, #longitudinal study
                                 multcorr=2,
                                 sig=0.05, #significance level
...
write.table(res3,"ANCOM_family_ANOVA_Treatment_adjCondition.txt",sep="\t",col.names=NA)
res4 <- res3[which(res3$detected_0.7==TRUE),] 

#### ANCOM test - Condition, no adjustment, KruskallWallis
comparison_test_treat=ANCOM.main(OTUdat=otu_test, #calling the OTU table
                                 Vardat=map_test, #calling the metadata
                                 adjusted=FALSE, #true if covariates are to be included for adjustment
                                 repeated=FALSE, #repeated measure
...
write.table(res5,"ANCOM_family_KruskallWallis_Condition.txt",sep="\t",col.names=NA)
res6 <- res5[which(res5$detected_0.7==TRUE),] 

#### ANCOM test - Condition, Adjusted by Treatment, ANOVA
comparison_test_treat=ANCOM.main(OTUdat=otu_test, #calling the OTU table
                                 Vardat=map_test, #calling the metadata
                                 adjusted=TRUE, #true if covariates are to be included for adjustment
                                 repeated=FALSE, #repeated measure
...
write.table(res7,"ANCOM_family_ANOVA_Condition_adjTreatment.txt",sep="\t",col.names=NA)
res8 <- res7[which(res7$detected_0.7==TRUE),] 

#### ANCOM test - Treated, no adjustment, KruskallWallis
comparison_test_treat=ANCOM.main(OTUdat=otu_test, #calling the OTU table
                                 Vardat=map_test, #calling the metadata
                                 adjusted=FALSE, #true if covariates are to be included for adjustment
                                 repeated=FALSE, #repeated measure
                                 main.var="treated", #main variable or fator
                                 adj.formula= NULL, #other factors to include
                                 repeat.var=FALSE, #repeated measure
                                 long = FALSE, #longitudinal study
                                 multcorr=2,
...
write.table(res9,"ANCOM_family_KruskallWallis_treated.txt",sep="\t",col.names=NA)
res10 <- res9[which(res9$detected_0.7==TRUE),] 

```

Now plot the ANCOM results: BS3 site only
...
dim(otu_test) #221 rows (samples) 130 columns (families)
otu_test_relabund <-  sweep(otu_test[,2:130], 1, rowSums(otu_test[,2:130]), '/')
otu_test_relnames <- cbind(map_test[,1:5],otu_test_relabund)
...
sig_dis <- select(otu_test_relnames, "Sample.ID", treatment, condition, month, treated, Cryomorphaceae, Vibrionaceae, "NS9.marine.group", Halomonadaceae, Rhodobacteraceae)
sig_long <- reshape2::melt(sig_dis, id.vars=c("Sample.ID","treatment","condition","month","treated"),variable.name="Family",value.name="Proportion")
sum_sig <- Rmisc::summarySE(sig_long, measurevar = "Proportion", groupvars = c("treatment","Family"), na.rm=TRUE)
sum_sig$treatment <- factor(sum_sig$treatment, levels = c("none","control paste and bag","probiotic paste and bag","resistant"))
treatcolors <- c("none"="#999999","control paste and bag"="#000066","probiotic paste and bag"="#993300","resistant"="#000000")


pdf("ANCOM_Family_Treatment_adjCondition_BS3.pdf")
...
Perform ANCOM analysis - only BS3 site -----GENUS level
...
get_taxa_unique(ps, "Genus") #219
dat <- tax_glom(ps, taxrank = "Genus")
datm <- psmelt(dat)
otud <- reshape2::dcast(datm, Sample ~ Genus, value.var = 'Abundance', fun.aggregate = sum)
colnames(otud)[1] <- "Sample.ID"
metadat <- sample_data(ps) 
metadat <- as.data.frame(as.matrix(metadat))
metadat <- tibble::rownames_to_column(metadat, "Sample.ID")
names(otud) <- make.names(names(otud))
otu_test <- otud
write.table(otu_test,"ANCOM_otu_test_genus.txt",sep="\t",col.names=NA)
metadat <- select(metadat, c("Sample.ID","treatment","condition","month","treated")) # select only columns of interest
map_test <- metadat
write.table(map_test,"ANCOM_map_test_genus.txt",sep="\t",col.names=NA)
...
comparison_test_treat=ANCOM.main(OTUdat=otu_test, #calling the OTU table
                                 ...
                                 prev.cut=0.90) #OTUs with proportion of zeroes greater than prev.cut are not included in the analysis
res1 <- comparison_test_treat$W.taxa #taxa that significantly vary across factor level of interest
write.table(res1,"ANCOM_genus_KruskallWallis_Treatment.txt",sep="\t",col.names=NA)
res2 <- res1[which(res1$detected_0.7==TRUE),] 
...
comparison_test_treat=ANCOM.main(OTUdat=otu_test, #calling the OTU table
                                 ...
                                 prev.cut=0.90) #OTUs with proportion of zeroes greater than prev.cut are not included in the analysis
res3 <- comparison_test_treat$W.taxa #taxa that significantly vary across factor level of interest
write.table(res3,"ANCOM_genus_ANOVA_Treatment_adjCondition.txt",sep="\t",col.names=NA)
res4 <- res3[which(res3$detected_0.7==TRUE),] 
...
comparison_test_treat=ANCOM.main(OTUdat=otu_test, #calling the OTU table
                                 ...
                                 prev.cut=0.90) #OTUs with proportion of zeroes greater than prev.cut are not included in the analysis
res5 <- comparison_test_treat$W.taxa #taxa that significantly vary across factor level of interest
write.table(res5,"ANCOM_genus_KruskallWallis_Condition.txt",sep="\t",col.names=NA)
res6 <- res5[which(res5$detected_0.7==TRUE),] 
...
comparison_test_treat=ANCOM.main(OTUdat=otu_test, #calling the OTU table
                                 ...
                                 prev.cut=0.90) #OTUs with proportion of zeroes greater than prev.cut are not included in the analysis
res7 <- comparison_test_treat$W.taxa #taxa that significantly vary across factor level of interest
write.table(res7,"ANCOM_genus_ANOVA_Condition_adjTreatment.txt",sep="\t",col.names=NA)
res8 <- res7[which(res7$detected_0.7==TRUE),] 
...
comparison_test_treat=ANCOM.main(OTUdat=otu_test, #calling the OTU table
                                 ...
                                 prev.cut=0.90) #OTUs with proportion of zeroes greater than prev.cut are not included in the analysis
res9 <- comparison_test_treat$W.taxa #taxa that significantly vary across factor level of interest
write.table(res9,"ANCOM_genus_KruskallWallis_treated.txt",sep="\t",col.names=NA)
res10 <- res9[which(res9$detected_0.7==TRUE),] 

```

Now plot the ANCOM results: BS3 site only - Genus level

```{r, echo=FALSE}

sig_sites <- glue::glue_collapse(droplevels(factor(res4$otu.names)), sep = ", ") #this is to get a list of the genera that are different
print(sig_sites)
#Ruegeria, NS4.marine.group, Rhodobacteraceae, Cryomorphaceae, NS9.marine.group, Vibrio, Halomonas, Photobacterium


#Calculate relative abundance
dim(otu_test) #221 rows (samples) 220 columns (219 genera)
otu_test_relabund <-  sweep(otu_test[,2:220], 1, rowSums(otu_test[,2:220]), '/')
otu_test_relnames <- cbind(map_test[,1:5],otu_test_relabund)

# Select only the significant differentially abundant families listed by sig_sites
# Note that any family names with punctuation must be enclosed with quotation marks!
sig_dis <- select(otu_test_relnames, "Sample.ID", treatment, condition, month, treated, Ruegeria, "NS4.marine.group", Rhodobacteraceae, Cryomorphaceae, "NS9.marine.group", Vibrio, Halomonas, Photobacterium)
sig_long <- reshape2::melt(sig_dis, id.vars=c("Sample.ID","treatment","condition","month","treated"),variable.name="Genus",value.name="Proportion")
sum_sig <- Rmisc::summarySE(sig_long, measurevar = "Proportion", groupvars = c("treatment","Genus"), na.rm=TRUE)
sum_sig$treatment <- factor(sum_sig$treatment, levels = c("none","control paste and bag","probiotic paste and bag","resistant"))
treatcolors <- c("none"="#999999","control paste and bag"="#000066","probiotic paste and bag"="#993300","resistant"="#000000")


pdf("ANCOM_Genus_Treatment_adjCondition_BS3.pdf")
gen <- ggplot(sum_sig, aes(x=Genus, y=Proportion))+
  geom_point(size=4, aes(color=treatment))+
  scale_color_manual(values=treatcolors)+
  coord_flip()+
  theme_bw()+
...
ps <- phyloseq(otu_table(otu, taxa_are_rows=FALSE),sample_data(samples),tax_table(taxon))
ps #456 taxa and 581 samples
bs2<-subset_samples(ps, site=="BS2")
bs2  #456 taxa and 200 samples
otu_bs2 = as(otu_table(bs2), "matrix")
taxon_bs2 = as(tax_table(bs2), "matrix")
meta_bs2 = as(sample_data(bs2), "matrix")
write.table(otu_bs2,"silva_nochloronomito_otu_table_bs2.txt",sep="\t",col.names=NA)
write.table(taxon_bs2,"silva_nochloronomito_taxa_table_bs2.txt",sep="\t",col.names=NA)
write.table(meta_bs2,"metadata_bs2.txt",sep="\t",col.names=NA)
...
sampledata = sample_data(samples)
ps <- phyloseq(otu_table(otu, taxa_are_rows=FALSE),sample_data(samples),tax_table(taxon))
ps  #456 taxa and 200 samples


#Perform ANCOM analysis - only BS2 site -----GENUS level
get_taxa_unique(ps, "Genus") #219
dat <- tax_glom(ps, taxrank = "Genus")
datm <- psmelt(dat)
otud <- reshape2::dcast(datm, Sample ~ Genus, value.var = 'Abundance', fun.aggregate = sum)
...
names(otud) <- make.names(names(otud))
otu_test <- otud
write.table(otu_test,"ANCOM_otu_test_genus_BS2.txt",sep="\t",col.names=NA)
metadat <- select(metadat, c("Sample.ID","treatment","condition","month","treated")) # select only columns of interest
map_test <- metadat
write.table(map_test,"ANCOM_map_test_genus_BS2.txt",sep="\t",col.names=NA)
...
comparison_test_treat=ANCOM.main(OTUdat=otu_test, #calling the OTU table
                                 ...
                                 prev.cut=0.90) #OTUs with proportion of zeroes greater than prev.cut are not included in the analysis
res1 <- comparison_test_treat$W.taxa #taxa that significantly vary across factor level of interest
write.table(res1,"ANCOM_genus_KruskallWallis_Treatment_BS2.txt",sep="\t",col.names=NA)
res2 <- res1[which(res1$detected_0.7==TRUE),] 
...
comparison_test_treat=ANCOM.main(OTUdat=otu_test, #calling the OTU table
                                 ...
                                 prev.cut=0.90) #OTUs with proportion of zeroes greater than prev.cut are not included in the analysis
res3 <- comparison_test_treat$W.taxa #taxa that significantly vary across factor level of interest
write.table(res3,"ANCOM_genus_ANOVA_Treatment_adjCondition_BS2.txt",sep="\t",col.names=NA)
res4 <- res3[which(res3$detected_0.7==TRUE),] 

sig_sites <- glue::glue_collapse(droplevels(factor(res4$otu.names)), sep = ", ") #this is to get a list of the genera that are different
print(sig_sites)
#Halodesulfovibrio, Roseibacillus, Rubritalea, Paramaledivibacter, Verrucomicrobiales, Shimia, ML602J.51, Oceanospirillum, Candidatus.Thiodiazotropha, Candidatus.Aquiluna, Psychrobium


#Calculate relative abundance
dim(otu_test) #200 rows (samples) 220 columns (219 genera)
otu_test_relabund <-  sweep(otu_test[,2:220], 1, rowSums(otu_test[,2:220]), '/')
otu_test_relnames <- cbind(map_test[,1:5],otu_test_relabund)

# Select only the significant differentially abundant families listed by sig_sites
# Note that any family names with punctuation must be enclosed with quotation marks!
sig_dis <- select(otu_test_relnames, "Sample.ID", treatment, condition, month, treated, Halodesulfovibrio, Roseibacillus, Rubritalea, Paramaledivibacter, Verrucomicrobiales, Shimia, "ML602J.51", Oceanospirillum, "Candidatus.Thiodiazotropha", "Candidatus.Aquiluna", Psychrobium)
sig_long <- reshape2::melt(sig_dis, id.vars=c("Sample.ID","treatment","condition","month","treated"),variable.name="Genus",value.name="Proportion")
sum_sig <- Rmisc::summarySE(sig_long, measurevar = "Proportion", groupvars = c("treatment","Genus"), na.rm=TRUE)
sum_sig$treatment <- factor(sum_sig$treatment, levels = c("none","control bag","control paste","probiotic bag","probiotic paste","resistant"))
treatcolors <- c("none"="#999999","control bag"="#56B4E9","control paste"="#0072B2","probiotic bag"="#E69F00","probiotic paste"="#D55E00","resistant"="#000000")

pdf("ANCOM_Genus_Treatment_adjCondition_BS2.pdf")
gen <- ggplot(sum_sig, aes(x=Genus, y=Proportion))+
  geom_point(size=4, aes(color=treatment))+
  scale_color_manual(values=treatcolors)+
  coord_flip()+
