...
```{r, echo=FALSE}
#### 1st sequencing run
path <- "~/Documents/Github/McH1-7_Probiotics_Field_Trials/cutadapt_NS2107_BS2_August"
list.files(path)
...
saveRDS(seqtab, "~/Documents/Github/McH1-7_Probiotics_Field_Trials/cutadapt_NS2107_BS2_August/seqtab.rds") 
```


```{r, echo=FALSE}
#### 2nd sequencing run
path <- "~/Documents/Github/McH1-7_Probiotics_Field_Trials/cutadapt_NS2172_BS2_October"
list.files(path)
fnFs <- sort(list.files(path, pattern="_R1_cut.fastq.gz", full.names = TRUE))
fnRs <- sort(list.files(path, pattern="_R2_cut.fastq.gz", full.names = TRUE))
sample.names <- sapply(strsplit(basename(fnFs), "_"), `[`, 1)
...
saveRDS(seqtab, "~/Documents/Github/McH1-7_Probiotics_Field_Trials/cutadapt_NS2172_BS2_October/seqtab.rds")
```


```{r, echo=FALSE}
#### 3rd sequencing run
path <- "~/Documents/Github/McH1-7_Probiotics_Field_Trials/cutadapt_NS2211_BS2_December"
list.files(path)
fnFs <- sort(list.files(path, pattern="_R1_cut.fastq.gz", full.names = TRUE))
fnRs <- sort(list.files(path, pattern="_R2_cut.fastq.gz", full.names = TRUE))
sample.names <- sapply(strsplit(basename(fnFs), "_"), `[`, 1)
...
saveRDS(seqtab, "~/Documents/Github/McH1-7_Probiotics_Field_Trials/cutadapt_NS2211_BS2_December/seqtab.rds")
```


```{r, echo=FALSE}
#### 4th sequencing run
path <- "~/Documents/Github/McH1-7_Probiotics_Field_Trials/cutadapt_NS2408_BS2_repeats"
list.files(path)
fnFs <- sort(list.files(path, pattern="_R1_cut.fastq.gz", full.names = TRUE))
fnRs <- sort(list.files(path, pattern="_R2_cut.fastq.gz", full.names = TRUE))
...
rownames(track) <- sample.names
head(track)
write.table(track, "dada_read_stats4.txt",sep="\t",col.names=NA)
saveRDS(seqtab, "~/Documents/Github/McH1-7_Probiotics_Field_Trials/cutadapt_NS2408_BS2_repeats/seqtab.rds")
```


Now that I have determined the ASV tables for all 4 sequencing runs, I can merge the ASV tables and remove chimera sequences.

```{r, echo=FALSE}
st1 <- readRDS("~/Documents/Github/McH1-7_Probiotics_Field_Trials/cutadapt_NS2107_BS2_August/seqtab.rds") 
st2 <- readRDS("~/Documents/Github/McH1-7_Probiotics_Field_Trials/cutadapt_NS2172_BS2_October/seqtab.rds") 
st3 <- readRDS("~/Documents/Github/McH1-7_Probiotics_Field_Trials/cutadapt_NS2211_BS2_December/seqtab.rds") 
st4 <- readRDS("~/Documents/Github/McH1-7_Probiotics_Field_Trials/cutadapt_NS2408_BS2_repeats/seqtab.rds") 
st.all <- mergeSequenceTables(st1, st2, st3, st4, repeats="sum") # You may get the message "Duplicated sample names detected in the sequence table row names." to let you know that there are duplicate names across samples - it is not an error, just a message. If you have run a sample on more than one sequencing run, the ASV counts will be added together.
...
stat4 <- read.table("dada_read_stats4.txt",sep="\t",header=TRUE, row.names=1)
...
saveRDS(seqtab.nochim, file="~/Documents/Github/McH1-7_Probiotics_Field_Trials/BS2.rds")
# RELOAD THE SAVED INFO FROM HERE (if you have closed the project):
#seqtab.nochim <- readRDS("~/Documents/Github/McH1-7_Probiotics_Field_Trials/BS2.rds")
```
...
```{r, echo=FALSE}
taxa <- assignTaxonomy(seqtab.nochim, "~/Documents/Github/McH1-7_Probiotics_Field_Trials/silva_nr99_v138.1_train_set.fa.gz", multithread=TRUE)
...
Now, time to explore the data. Old numbers in comments - update since addition of repeats (sequencing run 4)

```{r, echo=FALSE}

ps #20179 taxa and 197 samples
get_taxa_unique(ps, "Order") #427
get_taxa_unique(ps, "Class") #182
ps5<-filter_taxa(ps, function(x) mean(x) >5, TRUE)
ntaxa(ps5) #736
ps10<-filter_taxa(ps, function(x) mean(x) >10, TRUE)
ntaxa(ps10) #382
get_taxa_unique(ps, "Genus") #1699
get_taxa_unique(ps5, "Genus") #315
get_taxa_unique(ps10, "Genus") #189

# Now export filtered otu and taxa tables from phyloseq for future reference after removing sample blanks
ps5 = subset_samples(ps5, TagID != "blank")
ps5 #736 taxa and 194 samples
...
ps10 = subset_samples(ps10, TagID != "blank")
ps10
...
samples$Date<-factor(samples$Date,levels=c("A","O","J"))
samples$Treatment <- factor(samples$Treatment, levels = c("none","control bag","control paste", "probiotic bag","probiotic paste"))
treatcolors <- c("none"="#999999","control bag"="#56B4E9","control paste"="#0072B2","probiotic bag"="#E69F00", "probiotic paste"="#D55E00")

pdf("PCA.pdf",bg = "white",width=8.5)
p<-ggplot(df_out,aes(x=PC1,y=PC2,fill=samples$Treatment,shape=samples$Date))
...
p + labs(x=xlab, y=ylab, fill="Treatment", shape="Date") + coord_fixed()
dev.off()
...
health<-as.character(samples$HealthState)
tag<-as.character(samples$TagID)

# permanova between groups using Aitchison distance
dist.clr <- dist(E.clr)
perm<-adonis(dist.clr~treat*date,as(sample_data(ps),"data.frame"))
print(perm)
perm<-adonis(dist.clr~treat*health,as(sample_data(ps),"data.frame"))
print(perm)
perm<-adonis(dist.clr~health*date,as(sample_data(ps),"data.frame"))
print(perm)
perm<-adonis(dist.clr~tag*date,as(sample_data(ps),"data.frame"))
print(perm)
...
Let's try just diseased, with and without treatment
...
ps5 <- phyloseq(otu_table(otu, taxa_are_rows=FALSE), 
               sample_data(samples), 
               tax_table(taxon))
ps5 # 736 taxa, 194 samples
dd<-subset_samples(ps5, HealthState=="Disease")
dd  # 736 taxa, 78 samples
otu_dd = as(otu_table(dd), "matrix")
taxon_dd = as(tax_table(dd), "matrix")
meta_dd = as(sample_data(dd), "matrix")
write.table(otu_dd,"silva_nochloronomito_otu_table_dd.txt",sep="\t",col.names=NA)
write.table(taxon_dd,"silva_nochloronomito_taxa_table_dd.txt",sep="\t",col.names=NA)
write.table(meta_dd,"metadata_dd.txt",sep="\t",col.names=NA)
otu <- read.table("silva_nochloronomito_otu_table_dd.txt",sep="\t",header=TRUE, row.names=1)
taxon <- read.table("silva_nochloronomito_taxa_table_dd.txt",sep="\t",header=TRUE,row.names=1)
samples<-read.table("metadata_dd.txt",sep="\t",header=T,row.names=1)
...
pdf("PCA_dd.pdf",bg = "white",width=8.5)
p<-ggplot(df_out,aes(x=PC1,y=PC2,fill=samples$Treatment,shape=samples$Date))
p<-p+geom_point(size=5)+
  theme(axis.title = element_text(size=14))+
  theme(axis.text=element_text(size=12))+
...
# permanova between groups using Aitchison distance
dist.clr <- dist(E.clr)
perm<-adonis(dist.clr~treat*date,as(sample_data(dd),"data.frame"))
print(perm)
perm<-adonis(dist.clr~treat*tag,as(sample_data(dd),"data.frame"))
print(perm)
perm<-adonis(dist.clr~tag*date,as(sample_data(dd),"data.frame"))
print(perm)

```

Let's try just AH, with and without treatment
...
ps5 <- phyloseq(otu_table(otu, taxa_are_rows=FALSE), 
               ...
               tax_table(taxon))
ps5 # 736 taxa, 194 samples
hd<-subset_samples(ps5, HealthState=="Apparently Healthy")
hd  # 736 taxa, 91 samples
otu_hd = as(otu_table(hd), "matrix")
taxon_hd = as(tax_table(hd), "matrix")
meta_hd = as(sample_data(hd), "matrix")
write.table(otu_hd,"silva_nochloronomito_otu_table_hd.txt",sep="\t",col.names=NA)
write.table(taxon_hd,"silva_nochloronomito_taxa_table_hd.txt",sep="\t",col.names=NA)
write.table(meta_hd,"metadata_hd.txt",sep="\t",col.names=NA)
otu <- read.table("silva_nochloronomito_otu_table_hd.txt",sep="\t",header=TRUE, row.names=1)
taxon <- read.table("silva_nochloronomito_taxa_table_hd.txt",sep="\t",header=TRUE,row.names=1)
samples<-read.table("metadata_hd.txt",sep="\t",header=T,row.names=1)
...
pdf("PCA_hd.pdf",bg = "white",width=8.5)
p<-ggplot(df_out,aes(x=PC1,y=PC2,fill=samples$Treatment,shape=samples$Date))
p<-p+geom_point(size=5)+
  theme(axis.title = element_text(size=14))+
  theme(axis.text=element_text(size=12))+
...
date<-as.character(samples$Date)
tag<-as.character(samples$TagID)
treated<-as.character(samples$Treated)
...
perm<-adonis(dist.clr~tag*date,as(sample_data(dd),"data.frame"))
print(perm)
perm<-adonis(dist.clr~treated*date,as(sample_data(dd),"data.frame"))
print(perm)
...
ps5 #736 taxa and 194 samples
ps5_ra<-transform_sample_counts(ps5, function(OTU) OTU/sum(OTU))
ps5_ra #736 taxa and 194 samples
get_taxa_unique(ps5_ra, "Order") #100
get_taxa_unique(ps5_ra, "Class") #39
n <- 39
...
#treatcolors <- c("none"="#999999","control bag"="#56B4E9","control paste"="#0072B2","probiotic bag"="#E69F00", "probiotic paste"="#D55E00")
#condcolors <- c("Healthy"="coral", "Apparently Healthy"="maroon", "Disease"="magenta4")
samples$Date<-factor(samples$Date,levels=c("A","O","J"))
samples$Treatment <- factor(samples$Treatment, levels = c("none","control bag","control paste", "probiotic bag","probiotic paste"))
samples$HealthState<-factor(samples$HealthState,levels=c("Healthy","Apparently Healthy","Diease"))

pdf("barchart_Class.pdf",width=13)
p1=plot_bar(ps5_ra, fill="Class")+
  geom_bar(aes(fill=Class), stat="identity",position="stack")+
  facet_grid(.~HealthState,scales="free",space="free")+
...
```{r, echo=FALSE}
Pseudoalt<-subset_taxa(ps5_ra, Genus=="Pseudoalteromonas")
Pseudoalt #10 taxa and 194 samples
...
names(otu.Pseudoalt)[8]<-"ASV7"
names(otu.Pseudoalt)[9]<-"ASV8"
names(otu.Pseudoalt)[10]<-"ASV9"
names(otu.Pseudoalt)[11]<-"ASV10"
...
otu_long_p<-reshape2::melt(otu.Pseudoalt.meta,id.vars=c("Sample","TagID","HealthState","Treatment","Treated","AllPresent","Status","Date"),variable.name="ASV",value.name="Proportion")

#PaletteWoodsHole bog and rocky_beach
pseudoalt_colors<-c("ASV1"="#AC7070","ASV2"="#D6D9DA","ASV3"="#33291B","ASV4"="#C5C7C7","ASV5"="#937771","ASV6"="#5A4A35","ASV7"="#476AA7","ASV8"="#716347","ASV9"="#7299CE","ASV10"="#A2937A")
...
otu_long_p_ASV$Treatment <- factor(otu_long_p_ASV$Treatment, levels = c("none","control bag","control paste", "probiotic bag","probiotic paste"))

pdf("Pseudoalteromonas_ASVs.pdf",bg = "white",width=8,height=5)
...
```{r, echo=FALSE}
otu_long_p$Treatment <- factor(otu_long_p$Treatment, levels = c("none","control bag","control paste", "probiotic bag","probiotic paste"))
otu_long_p$Date<-factor(otu_long_p$Date,levels=c("A","O","J"))
labels <- c(A = "August", O = "October", J="January")
otu_long_p$HealthState<-factor(otu_long_p$HealthState,levels=c("Healthy","Apparently Healthy","Disease"))
condcolors <- c("Healthy"="#E69F00", "Apparently Healthy"="#999999", "Disease"="#000000")
...
model2<- aov(otu_long_p$Proportion~otu_long_p$Date)
summary(model2)
TukeyHSD(model2)

model3<- aov(otu_long_p$Proportion~otu_long_p$HealthState)
summary(model3)
TukeyHSD(model3)

pdf("Pseudoalteromonas_byTreatment_Date.pdf",width=8,height=6)
